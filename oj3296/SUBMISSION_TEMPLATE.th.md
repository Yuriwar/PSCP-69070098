...
```text
โจทย์ต้องการให้หาค่าเฉลี่ยของแม่สี RGB สองสีเพื่อหาค่าสีใหม่ที่ผสมกัน
- Input: มี 2 บรรทัด แต่ละบรรทัดรับค่าแม่สี R G B 3 ค่า คั่นด้วยเว้นวรรค
- Output: ค่าสี R G B ใหม่ที่ผสมกันแล้ว แสดงผลคั่นด้วย space 1 ช่อง โดยหาจากค่าเฉลี่ยแบบปัดเศษลงของแต่ละแม่สี
```
...
```text
Step 1: รับ input 2 บรรทัด โดยใช้ .split() แยกค่า R G B 
Step 2: แปลงค่าแต่ละตัวเป็นชนิด integer
Step 3: นำค่า R1+R2, G1+G2, B1+B2 มาบวกกันแล้วหารเอาเศษลงตัวด้วย 2 หาค่าเฉลี่ย
Step 4: พิมพ์ค่า R_new, G_new, B_new
```
...
```text
เหมือนกับแผนแรกทุกประการ เพราะโจทย์ตรงไปตรงมา เข้าใจได้ทันที
```
...
```text
ทดสอบกรณีค่าแม่สีบวกกันแล้วหารลงตัวพอดี (เป็นเลขคู่)
```
...
```text
ทดสอบกรณีค่าผลรวมเป็นเลขคี่ เพื่อดูว่าการปัดเศษด้วย // ทำงานถูกต้องหรือไม่
```
