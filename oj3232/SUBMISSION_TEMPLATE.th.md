...
```text
โจทย์ต้องการหาจำนวนครั้งในการกระโดดของกบไปยังระยะ Y โดยกบจะกระโดดครั้งแรก X และครั้งต่อไปจะลดลงทีละ 2
input : จำนวนเต็ม X Y
Output: แสดงจำนวนครั้งที่กระโดดได้ระยะทางสะสม >= Y หากโดดไม่ถึงให้แสดง -1
```
...
```text
Step 1: รับค่า X และ Y เข้ามาเป็นจำนวนเต็ม
Step 2: สร้างตัวแปรนับจำนวนครั้งกระโดด และระยะทางสะสม
Step 3: ใช้ลูปซ้ำเรื่อย ๆ ตรวจสอบก่อนว่า X <= 0 หรือไม่ ถ้าใช่แสดง -1 แล้วหยุดลูป ถ้าไม่ใช่ เพิ่มจำนวนครั้ง count_jump 1 และบวกระยะทาง X เข้าไปใน total_distance แล้วลดระยะกระโดด X ลงทีละ 2 ตรวจสอบว่า total_distance >= Y ยัง ถ้าถึงแล้วแสดงผล count_jump แล้วหยุดลูป
```
...
```text
ตรงกับแผนแรกที่คิดไว้ เพราะโจทย์ค่อนข้างเข้าใจง่ายคือ โดด X และ ลดทีละ 2 เมื่อถึงระยะ Y แสดงผลจำนวนกระโดด โดดไม่ถึงแสดง -1
```
