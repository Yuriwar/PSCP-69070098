...
```text
โจทย์ต้องการหาจำนวนร้านที่เปิดอยู่ในเวลา k ใดๆ 
input บรรทัดแรกคือจำนวนร้าน num และจำนวนครั้งที่ต้องการเช็ก จากนั้น num บรรทัดต่อมาเป็นเวลาเปิดปิดแต่ละร้าน บรรทัดสุดท้ายคือเวลา k จำนวน check ค่า คั่นด้วยช่องว่าง
output คือ จำนวนร้านที่เปิดในเวลา k แต่ละค่า พิมพ์บรรทัดเดียว เว้นด้วยช่องว่าง
* อยู่ในช่วง 0 - 1440 นาทีเท่านั้น
```
...
```text
Step 1: เนื่องจากขอบเขตเวลามีแค่นาทีที่ 0 ถึง 1440 (1441 ช่อง) ให้สร้างอาร์เรย์เปิด-ปิดไว้เก็บข้อมูล
Step 2: ใช้วิธีบวก 1 ตรง start และ ลบ 1 ตรง stop 
Step 3: วนลูปคำนวณ ตั้งแต่เวลา 0 ถึง 1440 เพื่อหาจำนวนร้านที่เปิดจริงในแต่ละนาที
Step 4: รับค่าเวลา k ที่ต้องการเช็กมาวนลูปอ่านตอบค่า open_count[k] ออกมาคั่นด้วยช่องว่าง
```
...
```text
วิธีสุดท้ายใช้ตรงกับแผนแรก ใช้อาร์เรย์ขนาด 1441 แล้วลงบันทึกการเปลี่ยนแปลง (+1 ที่ start และ -1 ที่ stop) จากนั้นอ่านค่าอาร์เรย์ที่ k
```
...
```text
ช่วงเวลาทั่วไป และเช็คว่าร้านปิดตาม input จริงไหม
```
...
```text
กรณีมีหลายร้านที่มีเวลาเปิดปิดทับซ้อนกัน
```
...
```text
กรณีช่วงเวลานั้นไม่มีร้านใดเปิด
```
