...
```text
โจทย์ให้สร้างสะพานยาวรวมเท่ากับ goal นิ้ว โดยมีอิฐเล็ก 1 นิ้ว a ก้อน กับใหญ่ 5 นิ้ว b ก้อน เงื่อนไขคือให้ใช้อิฐใหญ่ให้มากที่สุดก่อนค่อยใช้อิฐเล็ก 
รับ input สามบรรทัด คือ อิฐเล็ก a อิฐใหญ่ b ความยาวที่ต้องการ goal
output จำนวนอิฐเล็กที่ใช้น้อยที่สุด ถ้าสร้างไม่ได้ให้แสดง -1
```
...
Step 2: คำนวณหาจำนวนอิฐใหญ่ที่ต้องใช้มากที่สุด โดยเอา goal หารปัดเศษทิ้งด้วย 5 
Step 3: เช็คว่าอิฐใหญ่มีพอไหม ถ้ามีมากกว่า ให้ใช้แค่ที่คำนวณไว้ ถ้ามีน้อยกว่าก็ใช้ตามจำนวนที่มี 
Step 4: หาความยาวที่เหลือ โดยเอา goal - ความยาวอิฐที่ใช้ไป
Step 5: เช็กว่าอิฐเล็กมีพอไหม ถ้ามีพอตอบ ความยาวที่เหลือ ถ้ามีไม่พอแสดงผล -1
```
...
```text
วิธีสุดท้ายที่ใช้ตรงกับแผนแรกที่คิดไว้ โดยคิดคำนวณแบบตรงไปตรงมา
```
...
```text
กรณีทั่วไปอิฐใหญ่มีมากพอ และมีอิฐเล็กพอ
```
...
```text
กรณีใช้อิฐใหญ่ทั้งหมด แต่อิฐเล็กต่อให้ยาวถึงไม่พอ 
```
...
```text
กรณีไม่มีอิฐใหญ่เลย ใช้อิฐก้อนเล็กอย่างเดียว
```
