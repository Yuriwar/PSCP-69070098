...
```text
โจทย์ต้องการให้เราสร้างกรอบรูปดาว (*) ล้อมรอบข้อความ 5 บรรทัด
- Input: ข้อความจำนวน 5 บรรทัด
- Output: แสดงผลกรอบดาวที่มีขอบบน ขอบข้าง และขอบล่าง โดย:
    1. ขอบข้างซ้ายและขวาจะมีดาว (*) และเว้นวรรค 1 ช่อง
    2. ความยาวกรอบจะอิงตามความยาวของข้อความที่ยาวที่สุดใน 5 บรรทัดนั้น
    3. ข้อความบรรทัดที่สั้นกว่าจะถูกเติมด้วย space ด้านขวาให้ยาวเท่ากับข้อความที่ยาวที่สุด
    4. แถวบนสุดและแถวล่างสุดจะเป็นดาว (*) ยาวเท่ากับความยาวข้อความสูงสุด + 4 (บวกดาวและ space สองฝั่ง)
```
...
```text
Step 1: รับ input ข้อความเข้ามาเก็บไว้ใน list จำนวน  5 บรรทัด
Step 2: หาความยาวที่ยาวที่สุดใน list นั้นเพื่อนำไปคำนวณขนาดกรอบ
Step 3: ปริ้นท์ขอบบนด้วยดาว (*) ตามความยาวสูงสุด + 4
Step 4: วนลูปปรินท์แต่ละบรรทัด โดยจัดให้มี "* " ด้านหน้าและเติมช่องว่างด้านหลังข้อความให้ยาวเท่ากับคำที่ยาวที่สุด แล้วปิดด้วย " *"
Step 5: ปรินท์ขอบล่างด้วยดาว (*) แบบเดียวกับขอบบน
```
...
```text
คล้ายแผนแรก แต่มีการปรับปรุงส่วนการรับ input โดยใช้ try-except (EOFError) เพื่อป้องกันการผิดพลาดบรรทัดว่าง
```
...
```text
ทดสอบกรณีข้อความมีความยาวปกติทั่วไป
```
...
```text
ทดสอบกรณีที่บรรทัดแรกยาวที่สุด และบรรทัดอื่นเป็นสตริงว่างเปล่า
```
...
```text
ทดสอบกรณีทุกบรรทัดมีความยาวเท่ากันหมด
```
...
```text
ช่วยดูข้อผิดพลาดของ EOFerror
```

คุณยังทำอะไรด้วยตนเอง

```text
โค้ดส่วนอื่น ๆ ลอจิก ทำเองหมด
```
