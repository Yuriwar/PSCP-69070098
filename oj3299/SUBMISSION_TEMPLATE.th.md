...
```text
โจทย์ต้องการหาว่าต้นดอกไม้ต้นที่ N อยู่ในวงแถวที่เท่าไหร่
input: รับค่า L ความกว้างของแต่ละวงแถว และ N ลำดับของต้นดอกไม้ที่ต้องการหา
output: แสดงหมายเลขวงแถวที่ต้นไม้ N อยู่
```
...
```text
Step 1: รับค่า L และ N จาก input
Step 2: กำหนดค่าเริ่มต้นของจำนวนดอกไม้ในชั้นแรก current_band = L * (L+1) // 2 และให้หมายเลขชั้นเริ่มต้นเป็น 1
Step 3: ใช้ลูปเช็คว่า N มากกว่าจำนวนดอกไม้ในชั้นปัจจุบันหรือไม่ ถ้ามากกว่า ลบจำนวนดอกไม้ในชั้นปัจจุบันออกจาก N แล้วเพิ่มหมายเลขชั้น และเพิ่มขนาดความจุชั้นต่อไปอีก L*L
Step 4: ทำซ้ำจนกว่า N จะน้อยกว่าหรือเท่ากับความจุของชั้นนั้น แล้วแสดงผลค่า band
```
...
```text
เหมือนกับแผนแรกทุกประการ โดยใช้ลูป while หักลบจำนวนดอกไม้ทีละชั้น และอัปเดตความจุของชั้นถัดไปเรื่อยๆ เพื่อหาว่าลำดับ N ไปตกอยู่ในวงแถวใด
```
...
```text
ทดสอบกรณีที่ N อยู่ในวงแถวแรก (band 1) พอดี
```
...
```text
ทดสอบกรณีที่ N ข้ามไปยังวงแถวถัดไป (band 2)
```
...
```text
ทดสอบกรณีค่า N มีขนาดใหญ่ ซึ่งต้องวนลูปผ่านหลายวงแถว (band 3 ขึ้นไป)
```
